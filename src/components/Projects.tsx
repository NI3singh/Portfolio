...
const ProjectCard: React.FC<Project & { liveLink?: string; index: number; stars: number }> = ({
  ...
  return (
    ...
    >
      <div className={`
        relative h-full bg-surface/80 backdrop-blur-xl rounded-2xl overflow-hidden
        border border-glass/10 transition-all duration-500 group
        ${isHovered ? 'border-secondary/50 shadow-[0_0_30px_-5px_rgba(198,244,50,0.3)]' : 'hover:border-glass/20'}
      `}>
        ...
        <div className="relative p-6 sm:p-8 flex flex-col h-full z-10">
          ...
          <div className="mb-6">
            <div className="flex items-start justify-between gap-3 mb-4">
              ...
              <a
                ...
                className="flex-shrink-0 inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-glass/5 border border-glass/10 text-xs font-semibold text-text-primary hover:border-secondary/40 hover:text-secondary transition-colors"
              >
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {technologies.slice(0, 4).map((tech) => (
                <span
                  key={tech}
                  className="text-xs font-medium px-2.5 py-1 rounded-full bg-glass/5 text-text-primary border border-glass/5"
                >
                  {tech}
                </span>
              ))}
              {technologies.length > 4 && (
                <span className="text-xs font-medium px-2.5 py-1 rounded-full bg-glass/5 text-text-secondary border border-glass/5">
                  +{technologies.length - 4}
                </span>
              )}
            </div>
          ...
          {/* Footer Actions */}
          <div className="mt-auto pt-4 border-t border-glass/10 flex items-center justify-between gap-4">
            <a
              href={githubLink}
              target="_blank"
              rel="noopener noreferrer"
    ...
    <section id="projects" className="relative bg-transparent py-20 overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        >
          ...
          <button
            onClick={handlePrev}
            className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 z-20 p-3 rounded-full bg-surface/80 border border-glass/10 text-text-primary hover:bg-secondary hover:border-secondary hover:text-on-secondary transition-all duration-300 opacity-0 group-hover:opacity-100 focus:opacity-100 shadow-lg backdrop-blur-sm"
          ...
          <button
            onClick={handleNext}
            className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 z-20 p-3 rounded-full bg-surface/80 border border-glass/10 text-text-primary hover:bg-secondary hover:border-secondary hover:text-on-secondary transition-all duration-300 opacity-0 group-hover:opacity-100 focus:opacity-100 shadow-lg backdrop-blur-sm"
            ...
            return (
              <button
                ...
                className={`
                  ...
                  ${isActive ? 'w-8 bg-secondary' : 'w-2 bg-glass/30 hover:bg-glass/50'}
                `}
        ...
        >
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 px-8 py-3 rounded-full bg-glass/5 hover:bg-glass/10 border border-glass/10 hover:border-glass/20 text-text-primary font-medium transition-all duration-300 hover:scale-105"
          >
